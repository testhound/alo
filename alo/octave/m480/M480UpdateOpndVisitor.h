#ifndef _M480_UPDATE_OPND_VISITOR_H_
#define _M480_UPDATE_OPND_VISITOR_H_
...
namespace Alo
{
  ...
  class M480UpdateOpndVisitor : 
    public OctaveUpdateOpndVisitor,
    public M480Visitor
    {
    public:

      M480UpdateOpndVisitor(OctaveAlo &i,   	
			    shared_ptr<AltEntryFunction> fn);

      M480UpdateOpndVisitor(const M480UpdateOpndVisitor& rhs);
      const M480UpdateOpndVisitor& operator=(const M480UpdateOpndVisitor&);

      ~M480UpdateOpndVisitor(){}
    ...
    protected:
      ...
      shared_ptr<AltEntryFunction> getCurrentFn() const;
	...
	getHardRegister(unsigned int);

      shared_ptr<const Register> 
	getHardRegister(unsigned int, unsigned int);

      shared_ptr<const Register> 
	getLinkRegister(LinkFlag::Link_Flag_t lnk);

      shared_ptr<const Register> 
	getLinkedMemTypeRegister(OctaveMemory::MemType_t);

      void 
	setLinkRegisterUsage(const OctaveInstruction &i,
			     shared_ptr<const Register> rd);

      void 
	setMemRefRegisterUsage(const OctaveInstruction &i,
			       shared_ptr<const OctaveMemory> m);

      void 
	updateLinkedMemoryRegisters(OctaveInstruction &i,
				    shared_ptr<Register>);

      void 
	updateLinkInfo(unsigned int excLine,
		       const char* excFile,
      ...
      void 
	addMemRefRegOperands(OctaveInstruction &i,
			     shared_ptr<const OctaveMemory> mem);


      shared_ptr<const PrototypeDirective>
	searchForPrototype(unsigned int line,
			   const char* file,
			   const char* name);
	};
